...
Consumed by horizon-rs (via `horizon-cli` over `proposal.datomic`) and by
CriomOS through the horizon projection.

## Schema

The proposal schema is owned by horizon-rs. Records are **positional**
(no `field=value`); field order in the Datomic matches source-declaration
order in the Rust structs. Validated by `horizon-cli` on every use;
Nix consumers run `horizon-cli` and read the result via
`builtins.fromJSON` (no `builtins.fromDatomic` exists).

## Wire format

`proposal.datomic` is the wire form. There is no `datom.nix` and no
...
- Before pushing any `proposal.datomic` change, use the exact `horizon-rs` revision
